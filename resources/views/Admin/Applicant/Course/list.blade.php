                ...
                <div class="box-body">
                    <table id="example1" class="table table-bordered table-striped">
                        ...
                        @foreach($courses as $course)
                            <tr>
                                <td>
                                    <a href="{{ url('admin/applicant/course/'.$course['id'].'/detail') }}">
                                        {{ $course['courseName'] }}
                                    </a>
                                </td>
                                <td>{{ $course['time'] }}</td>
                                ...
                                <td>
                                    <a href="{{ url('admin/applicant/article/'.$course['id']).'/add' }}">
                                        <button type="button" class="btn btn-block btn-info btn-xs">添加文章</button></a>
                                </td>
                                <td>
                                    <a href="{{ url('admin/applicant/exercise/'.$course['id']).'/add' }}">
                                        <button type="button" class="btn btn-block btn-info btn-xs">添加题目</button></a>
                                </td>
                                <td>
                                    <a href="{{ url('admin/applicant/course/'.$course['id']).'/edit' }}">
...
<script>
    ...
    var hideCourses = function hideCourses (coursesId) {
        $.ajax({
            'url': '/admin/applicant/course/' + coursesId + '/hide',
            'method': 'patch',
