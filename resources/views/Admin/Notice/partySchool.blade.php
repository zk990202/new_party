                    ...
                    <table id="example1" class="table table-bordered table-striped">
                        ...
                        @foreach($notices as $notice)
                            <tr>
                                ...
                                <td>
                                    <a href="{{ url('admin/notice/party-school/'.$notice['id'].'/edit') }}">
                                        <button type="button" class="btn btn-block btn-info btn-xs">编辑</button></a>
                                </td>
                                <td>
                                    @if($notice['isTop'])
                                        <button id="down" type="button" class="btn btn-block btn-warning btn-xs" onclick="downNotice({{ $notice['id'] }});">取消</button>
                                    @else
                                        <button id="topUp" type="button" class="btn btn-block btn-success btn-xs" onclick="topUpNotice({{ $notice['id'] }});">置顶</button>
                                    @endif
                                </td>
                            </tr>
                        @endforeach
...
<script>
    ...
    var hideNotice = function hideNotice (noticeId) {
        $.ajax({
            'url': '/admin/notice/party-school/' + noticeId + '/hide',
            'method': 'patch',
    ...
    var topUpNotice = function (noticeId) {
        $.ajax({
            'url': '/admin/notice/party-school/' + noticeId + '/top-up',
            'method': 'patch',
            'success': function (data) {
                window.location.reload();
            }
