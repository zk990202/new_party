                    ...
                    <div class="box-body">
                        <table class="table table-striped table-bordered table-condensed">
                            ...
                            <tr>
                                ...
                                <td>
                                    @if(!$branch['secretary'])
                                        <a href="{{ url('manager/party-branch/'.$branch['id'].'/add-cadre/1') }}">
                                    ...
                                    @endif
                                    @if(!$branch['organizer'])
                                            <a href="{{ url('manager/party-branch/'.$branch['id'].'/add-cadre/2') }}">
                                        ...
                                        @endif
                                        @if(!$branch['propagator'])
                                            <a href="{{ url('manager/party-branch/'.$branch['id'].'/add-cadre/3') }}">
                            ...
                            <tr>
                                ...
                                <td>
                                    @if($branch['type'] == 4)
                                        <a href="{{ url('manager/party-branch/'.$branch['id'].'/member-add-mix-preview') }}">
                                            <button type="button" class="btn btn-success btn-xs">成员添加</button>
                                        </a>
                                    @else
                                        <a href="{{ url('manager/party-branch/'.$branch['id'].'/member-add') }}">
                                            <button type="button" class="btn btn-success btn-xs">成员添加</button>
                                        </a>
                                    @endif
                                    <a href="{{ url('manager/party-branch/'.$branch['id'].'/member-delete') }}">
                                        <button type="button" class="btn btn-danger btn-xs">删除成员</button>
                                    </a>
                                        <a href="{{ url('manager/party-branch/'.$branch['id'].'/member-list') }}">
                            ...
                            <tr>
                                <th>支部操作</th>
                                <td>
                                    <a href="{{ url('manager/party-branch/'.$branch['id'].'/edit') }}">
...
@section('func')
    ...
    <script>
        ...
        var deleteOrganizer = function (branchId) {
            $.ajax({
                url: '/manager/party-branch/' + branchId + '/delete-cadre/2',
                method: 'patch',
                data: 'form',
                dataType: 'json',
                success: function (data){
        ...
        var deletePropagator = function (branchId) {
            $.ajax({
                url: '/manager/party-branch/' + branchId + '/delete-cadre/3',
                method: 'patch',
                data: 'form',
                dataType: 'json',
                success: function (data){
        ...
        var deleteBranch = function (branchId) {
            $.ajax({
                url: '/manager/party-branch/' + branchId + '/delete',
                method: 'post',
                data: 'form',
                dataType: 'json',
                success: function (data){
                    if(data.success){
                        alert('删除成功');
                        window.location.href = '/manager/party-branch/list';
                    }else{
                        alert(data.message);
                    }
                }
