                        ...
                        <table id="example1" class="table table-bordered table-striped">
                            ...
                            @foreach($contents as $content)
                                <tr>
                                    ...
                                    <td>{{ $content['time'] }}</td>
                                    <td>{{ $content['filePath'] ? '有附件' : '无附件' }}</td>
                                    ...
                                    <td>
                                        @if($content['type'] == 7)
                                            <a href="{{ url('manager/theory-study/edit/article/'.$content['id']) }}">
                                            <button type="button" class="btn btn-block btn-info btn-xs">编辑</button></a>
                                        @elseif($content['type'] == 8)
                                            <a href="{{ url('manager/theory-study/edit/video/'.$content['id']) }}">
                                                <button type="button" class="btn btn-block btn-info btn-xs">编辑</button></a>
                                        @elseif($content['type'] == 9)
                                            <a href="{{ url('manager/theory-study/edit/eBook/'.$content['id']) }}">
                                                <button type="button" class="btn btn-block btn-info btn-xs">编辑</button></a>
                                        @endif
                                    </td>
                                </tr>
...
@section('func')
    ...
    <script>
        ...
        var hideContents = function hideContents (contentsId) {
            $.ajax({
                'url': '/manager/theory-study/' + contentsId + '/hide',
                'method': 'patch',
